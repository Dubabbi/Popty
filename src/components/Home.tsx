...
export function Home({ onNavigate, breakpoint }: HomeProps) {
  ...
  useEffect(() => {
    if (carouselRef.current && cardRefs.current[1]) {
      ...
      setTimeout(() => {
        cardRefs.current[1]?.scrollIntoView({ inline: 'center', block: 'nearest', behavior: 'auto' });
  ...
  return (
    <div style={{ paddingBottom: 'var(--space-8)' }}>
      ...
      {/* Trending Carousel */}
      <section style={{ padding: 'var(--space-6) var(--space-4)', position: 'relative', overflow: 'hidden' }}>
        {/* Background Overlay */}
        <div
          style={{
            position: 'absolute',
                  ...
                  return (
                    <div
                      ...
                      style={{
                        position: 'relative',
                        flexShrink: 0,
                        width: breakpoint === 'mobile' ? 'calc(100% - 64px)' : breakpoint === 'tablet' ? 'calc(100% - 120px)' : 'calc(100% - 200px)',
                        borderRadius: '32px',
                        overflow: 'hidden',
                          ...
                          <div>
                            <h2
                              style={{
                                ...
                                marginBottom: '16px',
                                color: '#FFFFFF',
                                fontSize: breakpoint === 'mobile' ? '44px' : breakpoint === 'tablet' ? '48px' : '52px',
                                fontWeight: 800,
                                lineHeight: 1.1,
      ...
      <section style={{ padding: '0 var(--space-4) var(--space-6)' }}>
        ...
        >
          {endingSoon.map((popup) => (
            <PopupCard key={popup.id} popup={popup} onClick={() => onNavigate('detail', popup.id)} />
          ))}
        </div>
      </section>
        ...
        {Object.entries(categoryPopups).map(([category, popups], index) => (
          <div key={category}>
            <div style={{ padding: '0 var(--space-4) var(--space-6)' }}>
              <h4 style={{ marginBottom: 'var(--space-3)', marginTop: 'var(--space-4)', color: 'var(--color-text-secondary)' }}>
      ...
      >
        ...
        <div>
          <h4 style={{ margin: 0, marginBottom: 'var(--space-1)', color: 'var(--color-text-primary)' }}>찾으시는 팝업이 없나요?</h4>
